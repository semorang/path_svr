...
#  if defined(USE_OPTIMAL_POINT_API)
      LOG_SET_FILEPATH("C:/__Data/optimal/log", "log_optimal");
#  elif defined(USE_PEDESTRIAN_DATA)
      LOG_SET_FILEPATH("C:/__Data/trekking/log", "log_pedestrian");
#  else
      LOG_SET_FILEPATH("C:/__Data/trekking/log", "log_trekking");
#  endif
#else
#  if defined(USE_OPTIMAL_POINT_API)
      LOG_SET_FILEPATH("/home/gmlee/__Data/optimal/data/log", "log_optimal");
#  elif defined(USE_PEDESTRIAN_DATA)
      // LOG_SET_FILEPATH("/home/ubuntu/walk_route/data/log", "log_pedestrian");
      // LOG_SET_FILEPATH("/home/gmlee/__Data/walk_route/data/log", "log_pedestrian");
#  else
      LOG_SET_FILEPATH("/home/ubuntu/walk_route/data/log", "log_trekking");
#  endif
#endif
...
#if defined(_WIN32)
#  if defined(USE_OPTIMAL_POINT_API)
      strInput = "C:/__Data/optimal";
#  elif defined(USE_PEDESTRIAN_DATA)
      strInput = "C:/__Data/walk_route/data";
#  else
      strInput = "C:/__Data/trekking";
#  endif
#else
#  if defined(USE_OPTIMAL_POINT_API)
      strInput = "/home/gmlee/__Data/optimal/data";
#  elif defined(USE_PEDESTRIAN_DATA)
      // strInput = "/home/ubuntu/walk_route/data/pedestrian.dat";
      strInput = "/home/gmlee/__Data/walk_route/data";
#  else
      strInput = "/home/gmlee/__Data/trecking/trecking.dat";
#  endif
#endif
      LOG_TRACE(LOG_DEBUG, "User input file not defined, will set default : %s", strInput.c_str());
   }
   else {
      LOG_TRACE(LOG_DEBUG, "Init path : %s", strInput.c_str());
   }
   ...
   m_pDataMgr.SetCacheCount(100);

	m_pFileMgr.SetDataMgr(&m_pDataMgr);
   m_pFileMgr.LoadData(strInput.c_str());
...
void GetOptimalPosition(const FunctionCallbackInfo<Value>& args) {
   ...
   if (args.Length() < 2) {
      ...
      bool nearAll = false;
      if (cnt >= 4) {
         nearAll = args[3].As<Boolean>()->Value();
      }

      // Expand Type
      bool isExpand = false;
      if (cnt >= 5) {
         isExpand = args[4].As<Boolean>()->Value();
      }

      LOG_TRACE(LOG_DEBUG, "Find Location lng:%f, lat:%f, ent_type:%d, near_all:%d", lng, lat, entType, nearAll);

      stOptimalPointInfo optInfo = {0, };

      uint32_t cntItems = m_pDataMgr.GetOptimalPointDataByPoint(lng, lat, &optInfo, 0, entType, nearAll);
