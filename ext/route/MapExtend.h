...
{
	std::unordered_map<uint64_t, stExtendInfo*> mapNode;
	std::unordered_map<uint64_t, stExtendInfo*> mapLink;
};
...
public:
	...
	const std::unordered_map<uint32_t, stExtendMeshInfo>* GetMapData(void); // 가능하면 쓰지 말자
...
private:
	std::unordered_map<uint32_t, stExtendMeshInfo> m_mapData;
};
