...
#if defined(USE_TREKKING_POINT_API) && defined(USE_FOREST_SAMPLE)
	// 인기도 가중치
	m_rpCost.forest.cost_popular = 0.1; // 0.1
	m_rpCost.forest.cost_comfortable = 0.01; // 0.01
...
void CRoutePlan::SetRouteCost(IN const uint32_t type, IN const DataCost* pCost)
{
	if (pCost != nullptr) {
...
const double CRoutePlan::GetCost(IN const stLinkInfo* pLink, IN const uint32_t dirTarget, IN const uint32_t opt, IN const double length, IN const uint32_t spd) // type, 0:보행자, 1:자전거 
{
	...
	if (dist > 0)
	{
		...
		if (pLink->base.link_type == TYPE_DATA_TREKKING)
		{
			...
			if (dirTarget == 1) { // 정방향 
				secCost = pLink->tre.fw_tm;
			}
			else {
				secCost = pLink->tre.bw_tm;
			}
#endif
...
const double CRoutePlan::GetTravelCost(IN const stLinkInfo* pLink, IN const stLinkInfo* pLinkPrev, IN const double cost, IN const int angle, IN const uint32_t type, IN const uint32_t opt, IN const uint32_t avoid) // type, 0:보행자, 1:자전거 
{
	...
	if (pLink != nullptr && pLink->length > 0)
	{
		if (pLink->base.link_type == TYPE_DATA_TREKKING)
		{
			...
			if (pLink->tre.popular <= 0) { 
				// 인기도 0인 길은, 가능하면(길이 없는 경우가 아니면) 탐색하지 말자
				secCost = INT64_MAX;
			...
			else if (opt == ROUTE_OPT_RECOMMENDED) {
				secCost = pLink->length * max(1, (4095 - pLink->tre.popular)) * m_rpCost.forest.cost_popular;
			}
			else if (opt == ROUTE_OPT_COMFORTABLE) {
				int slop = pLinkPrev->tre.slop - pLink->tre.slop;
				secCost += secCost * abs(slop) * m_rpCost.forest.cost_comfortable;
			}
		...
		else if (pLink->base.link_type == TYPE_DATA_PEDESTRIAN)
		{
			...
			if ((opt == ROUTE_OPT_COMFORTABLE)) {
				...
				//	secCost += angCost * 3;
				//}
				if ((pLink->ped.gate_type == 0) && (abs(ang180) > 10)) {
					// 회전을 할수록 가중치 적용
					angCost = abs(ang180) / 180.f;
					secCost += angCost * 3;
				}
		...
		else if (pLink->base.link_type == TYPE_DATA_VEHICLE)
		{
			if ((opt == ROUTE_OPT_FASTEST || opt == ROUTE_OPT_COMFORTABLE || opt == ROUTE_OPT_MAINROAD || opt == ROUTE_OPT_AUTOMATION)) {
				...
				if (pLink->length > 0)
				{
					...
					switch (pLink->veh.level) {
					case 0:
						avgSpd = m_rpCost.vehicle.cost_lv0; break;// 50; break; //100
					case 1:
						avgSpd = m_rpCost.vehicle.cost_lv1; break;// 50; break; //80
					case 2:
						avgSpd = m_rpCost.vehicle.cost_lv2; break;// 40; break; //80
					case 3:
						avgSpd = m_rpCost.vehicle.cost_lv3; break;// 30; break; //70
					case 4:
						avgSpd = m_rpCost.vehicle.cost_lv4; break;// 30; break; //60
					case 5:
						avgSpd = m_rpCost.vehicle.cost_lv5; break;// 25; break; //50
					case 6:
						avgSpd = m_rpCost.vehicle.cost_lv6; break;// 20; break; //40

					default:
						avgSpd = m_rpCost.vehicle.cost_lv7; break;// 15; break; //30
					} // switch
...
const int CRoutePlan::AddNewLinks(IN RouteInfo* pRouteInfo, IN const CandidateLink* pCurInfo, IN const int dir)
{
	...
	if (dir == 1) { // e 정방향
		...
		if (pCurInfo->nodeId.nid == pLink->enode_id.nid) {
			nextNodeId = pLink->snode_id;
			angStart = GetAngle(pLink, 2);
		}
		else if (pCurInfo->nodeId.nid == pLink->snode_id.nid) {
			nextNodeId = pLink->enode_id;
			angStart = GetAngle(pLink, 1);
		}
		else {
	...
	if (pNodeNext->base.point_type == TYPE_NODE_EDGE) {
		...
		pNodeNext = m_pDataMgr->GetVNodeDataById(oldKey);

		if (!pNodeNext)
		{
			LOG_TRACE(LOG_ERROR, "Failed, can't find next edge node, next edge node id:%lld", oldKey.llid);
			return -1;
		}
	}
	else if (pLink->base.link_type == TYPE_DATA_VEHICLE) {
 	...
 	for (uint32_t ii = 0; ii < pNodeNext->base.connnode_count; ii++)
	{
		...
		if (pLink->base.link_type == TYPE_DATA_VEHICLE) {
			...
			else if (pNodeNext->base.point_type == TYPE_NODE_END) { // 단점 노드일 경우
				if ((pLink->veh.link_type != 2) && (pLink->veh.pass_code == 1)) { // 본선분리 아니고, 일방 아닌, 통행 가능일 때
					retPassCode = 1; // 유턴가능 코드로 적용
					costTreavel = 1000.; // 단점 유턴의 경우, 비용을 높게 주자
				}
			}
#endif
		}
		...
		//else
		{
			...
			if (!IsVisitedLink(pRouteInfo, candidateId)) { // && !IsAddedLink(pLink->link_id)) {
				...
				{// 다음 링크의 snode 연결
					...
					//angDiff = angStart - GetAngle(pLinkNext, 2/*pLinkNext->link_id.dir*/);
					angDiff = GetAngle(pLinkNext, 2, false);
					dirTarget = 1; // 정
				}
				else
				{// 다음 링크의 enode 연결
					...
					//angDiff = angStart - GetAngle(pLinkNext, 1/*pLinkNext->link_id.dir*/);
					angDiff = GetAngle(pLinkNext, 1, false);
					dirTarget = 2; // 역
				}
				angDiff = abs(180 - angStart + angDiff + 360) % 360;
...
#	endif
#else
				heuristicCost = toDestStraightDist * VAL_HEURISTIC_VEHICLE_FACTOR;//VAL_HEURISTIC_PEDESTRIAN_FACTOR;
#endif
				...
				// 트리 등록
				costReal = GetCost(pLinkNext, dirTarget, pRouteInfo->RouteOption, 0, 0);
				costTreavel += GetTravelCost(pLinkNext, pLink, costReal, angDiff, pRouteInfo->RouteOption, pRouteInfo->RouteOption, pRouteInfo->AvoidOption); // 이전 링크/노드(단점 유턴) 속성에 의해 비용이 높아진 상태 반영			
		...
		//else
		{
			...
			if (pRouteInfo->mRoutePass.find(candidateId.llid) == pRouteInfo->mRoutePass.end()) 
			{
				...
				{// 다음 링크의 snode 연결
					...
					//angDiff = angStart - GetAngle(pLinkNext, 2);
					angDiff = GetAngle(pLinkNext, 2, false);
					dirTarget = 1; // 정
				}
				else
				{// 다음 링크의 enode 연결
					...
					//angDiff = angStart - GetAngle(pLinkNext, 1);
					angDiff = GetAngle(pLinkNext, 1, false);
					dirTarget = 2; // 역
				}
				angDiff = abs(180 - angStart + angDiff + 360) % 360;
...
const bool CRoutePlan::SetRouteLinkInfo(IN const SPoint& ptPosition, const IN KeyID keyLink, IN const bool isStart, IN const bool isDirIgnore, OUT RouteLinkInfo* pRouteLinkInfo)
{
...
#endif
			{
				...
				if (pNode && (pNode->base.connnode_count >= 1)) {
					for (int ii = 0; ii < pNode->base.connnode_count; ii++) {
						if ((isStart && getNextPassCode(pLink->link_id, pNode->connnodes[ii], pNode, 0, m_pDataMgr) == true) ||
							(!isStart && getPrevPassCode(pLink->link_id, pNode->connnodes[ii], pNode, 0, m_pDataMgr) == true)) {
							isPassed = true;
							break;
						}
					} // for
...
const int CRoutePlan::DoRoute(IN const uint32_t reqId, IN const SPoint ptStart, IN const SPoint ptEnd, IN const KeyID sLink, IN const KeyID eLink, IN const uint32_t routeOpt, IN const uint32_t avoidOpt, IN const bool ignoreStartDir, IN const bool ignoreEndDir, OUT RouteResultInfo* pRouteResult)
{
	RouteInfo routeInfo;
	...
	// 시작점 정보
	if (!SetRouteLinkInfo(ptStart, sLink, true, ignoreStartDir, &routeInfo.StartLinkInfo)) {
	...
	// 종료점 정보
	if (!SetRouteLinkInfo(ptEnd, eLink, false, ignoreEndDir, &routeInfo.EndLinkInfo)) {
...
#pragma omp parallel for
	...
	for (int ii = 0; ii < cntPoints - 1; ii++)
	{
		...
		if (!SetRouteLinkInfo(ptStart, sLink, true, isDirIgnore, &routeInfo.StartLinkInfo)) {
			LOG_TRACE(LOG_ERROR, "Failed, set start link info");
		...
		if (!SetRouteLinkInfo(ptEnd, eLink, false, isDirIgnore, &routeInfo.EndLinkInfo)) {
			LOG_TRACE(LOG_ERROR, "Failed, set end link info");
		...
		if (ret != ROUTE_RESULT_SUCCESS) {
			...
			failedInfo.type = routeInfo.EndLinkInfo.LinkGuideType;
...
const int CRoutePlan::DoTabulate(IN const RequestRouteInfo* pReqInfo, OUT RouteTable** ppResultTables)
{
	...
	for (int ii = 0; ii < cntPoints; ii++) {
		...
		// 각 지점의 매칭 정보
		if (!SetRouteLinkInfo(ptRequest, keyLink, false, pReqInfo->WayDirIgnore, &newInfo)) {
...
const int CRoutePlan::MakeRoute(IN const int idx, IN RouteInfo* pRouteInfo, OUT RouteResultInfo* pRouteResult)
{
	...
	if (pRouteInfo->StartLinkInfo.LinkDir == 0 || (((pLink->link_id.dir == 0) || (pLink->link_id.dir == 2)) && (pRouteInfo->StartLinkInfo.LinkDir == 2))) // 양방향 or 역방향
	{
		...
		pItem->timeReal = GetCost(pLink, candidateId.parents_id, pRouteInfo->RouteOption, pRouteInfo->StartLinkInfo.LinkDistToS, 0); // 실 주행 시간
	...
	if (pRouteInfo->StartLinkInfo.LinkDir == 0 || (((pLink->link_id.dir == 0) || (pLink->link_id.dir == 1)) && (pRouteInfo->StartLinkInfo.LinkDir == 1))) // 양방향 or 정방향
	{
		...
		pItem->timeReal = GetCost(pLink, candidateId.parents_id, pRouteInfo->RouteOption, pRouteInfo->StartLinkInfo.LinkDistToE, 0); // 실 주행 시간
	...
	while (!pRouteInfo->pqDijkstra.empty())
	{
		...
		if ((pCurInfo->linkId.llid == pRouteInfo->EndLinkInfo.LinkId.llid) && // 목적지 링크 이면서
			((pRouteInfo->EndLinkInfo.LinkDir == 0) || // 방향성이 없거나
			 (pRouteInfo->StartDirIgnore || pRouteInfo->EndDirIgnore) || // 방향성 무시거나
			 ((((pRouteInfo->EndLinkInfo.LinkId == pRouteInfo->StartLinkInfo.LinkId) && (nProcess <= 1) && // 출도착 동일 + 링크 시작이면서, 
			    (((pRouteInfo->EndLinkInfo.LinkDir == 1 && pCurInfo->dir == 1) && // 진행방향(출발->도착) 일때
			      (pRouteInfo->StartLinkInfo.LinkDistToS <= pRouteInfo->EndLinkInfo.LinkDistToS)) || // E -> S
			     ((pRouteInfo->EndLinkInfo.LinkDir == 2 && pCurInfo->dir == 2) && // 진행방향(출발->도착) 일때
			      (pRouteInfo->StartLinkInfo.LinkDistToE <= pRouteInfo->EndLinkInfo.LinkDistToE))) // S -> E
		     ))) || ((pCurInfo->dir == pRouteInfo->EndLinkInfo.LinkDir) && (nProcess > 1))) || // 시작 아니고, 동일 방향이거나
			(cntMaxExtraSearch < cntExtraSearch)) // 최대 목적지 도착 횟수를 넘어서면
		{
			...
			//ret = MakeRouteResult(pRouteInfo, pRouteResult);
	...
	if (ret != ROUTE_RESULT_SUCCESS) {
		...
		LOG_TRACE(LOG_INFO, "Failed, req_idx:%d, start(link:%d, coord:%.6f,%.6f), end(link:%d, coord:%.6f,%.6f)", idx,
...
const uint32_t CRoutePlan::CheckStartDirectionMaching(IN const stLinkInfo* pLink, IN const RouteLinkInfo* pRoutLinkInfo, IN const int32_t routeOpt, OUT vector<CandidateLink>& vtCandidateInfo)
{
		...
		)
	{
		...
		pItem.timeReal = GetCost(pLink, candidateId.parents_id, routeOpt, pRoutLinkInfo->LinkDistToS, 0); // 실 주행 시간
		...
		)
	{
		...
		pItem.timeReal = GetCost(pLink, candidateId.parents_id, routeOpt, pRoutLinkInfo->LinkDistToE, 0); // 실 주행 시간
...
const uint32_t CRoutePlan::CheckEndDirectionMaching(IN const stLinkInfo* pLink, IN const stNodeInfo* pSNode, IN const stNodeInfo* pENode, IN const RouteLinkInfo* pRoutLinkInfo, IN const int32_t routeOpt, OUT vector<CandidateLink>& vtCandidateInfo)
{
	...
	{
		// S 노드가 단점 아닌지 확인
		if (pSNode && pSNode->base.point_type != TYPE_NODE_END && pSNode->base.connnode_count >= 2) {
			...
			for (int ii = 0; ii < pSNode->base.connnode_count; ii++) {
				...
				pItem.nodeId = pLink->snode_id;	// 노드 ID
				pItem.distReal = pRoutLinkInfo->LinkDistToS; // 실 거리
				pItem.timeReal = GetCost(pLink, pRoutLinkInfo->LinkDir, routeOpt, pRoutLinkInfo->LinkDistToS, 0); // 실 주행 시간
				pItem.costTreavel = pRoutLinkInfo->LinkDistToS;	// 계산 비용
				pItem.costHeuristic = pRoutLinkInfo->LinkDistToS;	// 가중치 계산 비용
				pItem.depth = 0;	// 탐색 깊이
				pItem.visited = false; // 방문 여부
...
#else // #if defined(USE_TREKKING_DATA) 
#	if defined(USE_P2P_DATA)
	if (dir == 1) { // 정
...
#	endif
#endif // #if defined(USE_TREKKING_DATA) 
...
#pragma omp parallel for
	...
	for (int ii = 0; ii < cntRows; ii++) 
	{
		...
		while (!pBase->pqDijkstra.empty())
		{
			...
			if (cntDestination <= cntGoal)
			{
				...
				// release mem
				for (; !pBase->pqDijkstra.empty(); ) {
					delete pBase->pqDijkstra.top();
					pBase->pqDijkstra.pop();
				}

				if (!pBase->mRoutePass.empty()) {
					pBase->mRoutePass.clear();
					unordered_map<uint64_t, bool>().swap(pBase->mRoutePass);
				}
				...
				if (pBase->pqDijkstra.empty() && (cntDestination != cntGoal) && !isRetry) {

					// 시작점이 방향성을 가지고 있으면 반대로 설정해 한번더 시도
					if (cntGoal <= 0) {
						if ((baseTablesStart[ii].vtCandidateLink.size() == 1) && (baseTablesStart[ii].vtCandidateLink[0].dir != 0)) {
							isRetry = 1;
							...
							// 시작 지점 정보 재등록
							CandidateLink* pNew = new CandidateLink();
							memcpy(pNew, &baseTablesStart[ii].vtCandidateLink[0], sizeof(CandidateLink));
							// 출발지 진행 방향 바꾸기
							int oldDir = pNew->dir;
							pNew->dir = (pNew->dir == 1) ? pNew->dir = 2 : pNew->dir = 1;

							pBase->pqDijkstra.emplace(pNew);

							LOG_TRACE(LOG_DEBUG, "Retry, make table, rows[%02d], change start link dir, tile:%d, id:%d, dir:%d->%d", ii, pNew->linkId.tile_id, pNew->linkId.nid, oldDir, pNew->dir);
						}
					}
					// 도착지점이 방향성을 가지고 있으면 반대로 설정해 한번더 시도
						...
						if ((baseTablesStart[ii].vtCandidateLink.size() == 1) && (baseTablesStart[ii].vtCandidateLink[0].dir != 0)) {
							...
							// 시작 지점 정보 재등록
							pBase->pqDijkstra.emplace(&baseTablesStart[ii].vtCandidateLink[0]);
		...
		if (cntDestination != cntGoal) {
			if (isRetry) {
				...
				for (int idx = 0; idx < cntDestination; idx++) {
					if (ppResultTables[ii][idx].nUsable == false) {
						LOG_TRACE(LOG_WARNING, "Failed, can't retry make table, rows[%02d][%02d], link dir, tile:%d, id:%d, dir:%d", ii, idx, baseTablesStart[ii].vtCandidateLink[0].linkId.tile_id, baseTablesStart[ii].vtCandidateLink[0].linkId.nid, baseTablesStart[ii].vtCandidateLink[0].dir);
					}
				}
			}
			else {
	...
	LOG_TRACE(LOG_DEBUG, "Finish make table, rows:%d", cntRows);
...
const int CRoutePlan::MakeRouteResult(IN RouteInfo* pRouteInfo, OUT RouteResultInfo* pRouteResult)
{
		...
		pRouteInfo->StartLinkInfo.LinkDir == pRouteInfo->EndLinkInfo.LinkDir)
	{
		...
		if (pRouteInfo->StartLinkInfo.LinkSplitIdx != pRouteInfo->EndLinkInfo.LinkSplitIdx) {
			if (pRouteInfo->StartLinkInfo.LinkDir == 2)
			{
				for (int ii = pRouteInfo->StartLinkInfo.LinkSplitIdx + 1; ii < pRouteInfo->StartLinkInfo.LinkSplitIdx; ii++) {
			...
			{
				for (int ii = pRouteInfo->EndLinkInfo.LinkSplitIdx + 1; ii < pRouteInfo->EndLinkInfo.LinkSplitIdx; ii++) {
					pRouteResult->LinkVertex.emplace_back(pRouteInfo->EndLinkInfo.LinkVtxToE[ii]);
				}
			}
			// 양방향
			else
	...
	else
	{
		...
		currentLinkInfo.angle = 0;
		prevDir = GetAngle(pLink, pCandidateStartLink->dir);
		...
		for (int ii = cntLink - 2; ii > 0; --ii)
		{
			...
			// angle
			tmpAng = GetAngle(pLink, pCandidateLink->dir, false);
			// 주행 각도는 링크 시작값으로 주기에 최초 링크는 각도 무시.
			//currentLinkInfo.angle = abs(prevDir - tmpAng) % 360;
			//currentLinkInfo.angle = abs(prevDir - tmpAng - 180) % 360; 
			currentLinkInfo.angle = abs(180 - prevDir + tmpAng + 360) % 360;
			prevDir = GetAngle(pLink, pCandidateLink->dir);
		...
		// angle
		tmpAng = GetAngle(pLink, pCandidateEndLink->dir, false);
		// 주행 각도는 링크 시작값으로 주기에 최초 링크는 각도 무시.
		//currentLinkInfo.angle = abs(prevDir - tmpAng) % 360;
		currentLinkInfo.angle = abs(180 - prevDir + tmpAng + 360) % 360;
		prevDir = GetAngle(pLink, pCandidateEndLink->dir);
