...
public:
	...
	// TTL
	bool AddTTLData(IN const uint32_t ttl_nid, IN const uint32_t tile_nid, IN const uint32_t link_nid, IN const uint8_t dir);
	bool UpdateTTLData(IN const uint64_t ttlId, IN const uint8_t speed, uint32_t timestamp);
