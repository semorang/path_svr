...
// Second: The C++ Standard allows explicit specialization 
// only in namespace-scope.
//
...
sizeof(::Loki::TL::Private::TList_is_not_legal_Typelist  \
< (bool) (::Loki::TL::Private::IsTypelist<TList>::value) >)> \
...
////////////////////////////////////////////////////////////////////////////////
      ...
      struct IsTypelist {
        ...
        public:
          enum {
            temp1  = sizeof(check(Type2Type<T>())) == 
              sizeof(TypeTag<1>::X) ? Typelist_ID : NoneList_ID,
            temp2  = sizeof(check2(Type2Type<T>())) == 
              sizeof(TypeTag<2>::X) ? AtomList_ID : NoneList_ID,
            temp4  = temp2 ? Typelist_ID :NoneList_ID,
            temp3  = sizeof(check3(Type2Type<T>())) == 
              sizeof(TypeTag<3>::X) ? NullType_ID : NoneList_ID,
          ...
          <
            ...
            Typelist_tag,
            typename Select< type_id == 
              NullType_ID, NullType_tag, NoneList_tag>::Result
          >
          ::Result type_tag;
      };
...
// MakeTypeList-Template from Rani Sharoni's VC 7 port.
    template
    <  class T1  = NullType, class T2  = NullType, class T3  = NullType,
...
namespace Private {
  ...
  struct TypeAt {
    typedef typename 
      Private::TypeAtImpl<Index>::template In<TList>::Result Result;
  };
...
namespace Private {
  ...
  struct EraseAllImpl {  
    ...
    // TList::Head as its head and the result of 
    // applying EraseAll to TList::Tail
    // as its tail.
    template <class T>
    struct In {
      ASSERT_TYPELIST(TList);
  ...
  struct EraseAll {
    typedef typename 
      Private::EraseAllImpl<TList>::template In<T>::Result Result;
  };
...
// Arranges all the types in a typelist so that 
// the most derived types appear first
// Invocation (TList is a typelist):
// DerivedToFront<TList>::Result
// returns the reordered TList
////////////////////////////////////////////////////////////////////////////////
