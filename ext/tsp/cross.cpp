...
void TCross::setABcycle( const TIndi& tPa1, const TIndi& tPa2, int flagC[ 10 ], int numOfKids ){
	...
	while(koritsuMany!=0){
		...
		while(flagCycle==0){
			fPosiCurr++;
			if (fRoute.size() <= fPosiCurr) goto RETURN;
			pr=ci;
			switch(prType){
			case 1:
...
void TCross::changeSol( TIndi& tKid, int ABnum, int type ){
	...
	for(j=0;j<cem/2;j++){
		...
		if (fSegPosiList.size() <= fNumOfSPL) break;
...
void TCross::makeCompleteSol( TIndi& tKid ){
	...
	while( fNumOfUnit != 1 ){
		...
		//nearMax = 10;	// N_near
		nearMax = min(10, eval->fNearNumMax);
	...
	RESTART:
		...
		//if (a1 == -1 && nearMax == eval->fNearNumMax) {
		//	nearMax = eval->fNearNumMax * 3;
		//	goto RESTART;
		//}
		//else if (a1 == -1 && nearMax == eval->fNearNumMax * 3) {
