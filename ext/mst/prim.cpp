...
Travel mst_manhattan_branch_and_bound(IN const RouteTable** ppResultTables, IN const int count, int start)
{
	...
	if (ppResultTables && count > 0 && start >= 0) {
		...
		while (!pq.empty()) {
			...
			for (int i = 0; i < count; ++i) {
				if (getVisited(visited, i) == false) {
					...
					new_travel.cost = cost + ppResultTables[cur][i].dbTotalCost;
#else
					new_travel.cost = cost + ppResultTables[cur][i].nTotalDist;
#endif
					new_travel.idx = i;
...
Travel tsp_manhattan_branch_and_bound(IN const RouteTable** ppResultTables, IN const int count, int start)
{
	Travel best_path;
...
#if defined(USE_REAL_ROUTE_TSP_COST)
				best = cost + ppResultTables[cur][start].dbTotalCost;
#else
				best = cost + ppResultTables[cur][start].nTotalDist;
#endif				
				best_path = path;
				continue;
			}
...
#if defined(USE_REAL_ROUTE_TSP_COST)
					pq.push({ cost + ppResultTables[cur][i].dbTotalCost,{ i, new_path } });
#else
					pq.push({ cost + ppResultTables[cur][i].nTotalDist,{ i, new_path } });
#endif					
				}
			}
		}
...
int least(IN const RouteTable** ppResultTables, IN const int count, int c) {
	int i, nc = 999;
	uint32_t kmin = 0;
	...
	for (i = 0; i < count; i++) {
#if defined(USE_REAL_ROUTE_TSP_COST)
		if ((ppResultTables[c][i].dbTotalCost != 0) && (getVisited(best_path.visited, i) == false)) {
			if (ppResultTables[c][i].dbTotalCost + ppResultTables[i][c].dbTotalCost < min) {
				min = ppResultTables[i][0].dbTotalCost + ppResultTables[c][i].dbTotalCost;
				kmin = ppResultTables[c][i].dbTotalCost;
				nc = i;
			}
		}
#else
		if ((ppResultTables[c][i].nTotalDist != 0) && (getVisited(best_path.visited, i) == false)) {
			if (ppResultTables[c][i].nTotalDist + ppResultTables[i][c].nTotalDist < min) {
				min = ppResultTables[i][0].nTotalDist + ppResultTables[c][i].nTotalDist;
				kmin = ppResultTables[c][i].nTotalDist;
				nc = i;
			}
		}
#endif		
...
void mincost(IN const RouteTable** ppResultTables, IN const int count, int start) {
	int i, ncity;
	...
	ncity = least(ppResultTables, count, start);

	if (ncity == 999) {
		ncity = 0;
#if defined(USE_REAL_ROUTE_TSP_COST)
		best_path.cost += ppResultTables[start][ncity].dbTotalCost;
#else
		best_path.cost += ppResultTables[start][ncity].nTotalDist;
#endif
	...
	mincost(ppResultTables, count, ncity);
}
...
Travel mst_manhattan_branch_and_bound2(IN const RouteTable** ppResultTables, IN const int count, int start) {
	...
	mincost(ppResultTables, count, start);
